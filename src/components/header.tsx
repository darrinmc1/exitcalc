      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="flex h-16 items-center justify-between">
          <Link href="/" className="flex items-center gap-2 group">
            ...
            <span className="font-[var(--font-outfit)] text-lg font-bold gradient-text-teal">
              {siteConfig.name}
            </span>
          </Link>
          ...
          <nav className="hidden md:flex items-center gap-1">
            {siteConfig.nav.marketing.map((link) => (
              <Link
                key={link.href}
                href={link.href}
                className="px-4 py-2 text-sm font-medium text-slate-300 hover:text-teal-400 rounded-lg hover:bg-white/5 transition-all duration-200"
              >
                {link.label}
              </Link>
            ))}
          ...
          <div className="px-4 py-4 space-y-2">
            {siteConfig.nav.marketing.map((link) => (
              <Link
                ...
                className="block px-4 py-3 text-sm font-medium text-slate-300 hover:text-teal-400 rounded-lg hover:bg-white/5 transition-all"
              >
                {link.label}
              </Link>
            ))}
            <div className="pt-4 border-t border-white/10 space-y-2">
              {isLoaded && !isSignedIn && (
                <>
                  ...
                  <SignUpButton mode="modal">
                    <button className="block w-full px-4 py-3 text-sm font-bold text-white text-center rounded-xl bg-gradient-to-r from-teal-600 to-teal-500">
                      {siteConfig.copy.ctaButton}
                    </button>
                  </SignUpButton>
                </>
